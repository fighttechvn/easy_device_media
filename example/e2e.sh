...
run_flutter_step() {
  ...
  if [ "$status" -ne 0 ]; then
    if grep -q "All tests passed" "$step_log" && grep -q "Unable to terminate" "$step_log"; then
      echo "Ignoring Flutter cleanup failure after successful test run." | tee -a "$LOG_FILE"
    else
      rm -f "$step_log"
      return "$status"
    fi
